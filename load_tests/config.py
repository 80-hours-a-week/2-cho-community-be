...
seed_accounts.py가 생성한 계정 패턴(user{i}@example.com / Test1234!)을 사용합니다.

대상 환경: EKS (NLB → Ingress → FastAPI Pod ×2~4, HPA 70% CPU)
"""

# ── 계정 설정 ──────────────────────────────────────────────
# seed_accounts.py 기준: user1@example.com ~ user250@example.com
...
# ── 요청 타임아웃 (초) ──────────────────────────────────────
# EKS: 콜드 스타트 없음. NLB → Pod 직결이므로 응답 빠름
REQUEST_TIMEOUT = 10
